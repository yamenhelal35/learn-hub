...
const AddTodo = ({ addTodo }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (todoText.trim()) {
      ...
      const response = await axios.post('http://localhost:8004/todos', newTodo, { withCredentials: true });
  ...
  return (
    <form onSubmit={handleSubmit} className="flex justify-center mb-4">
      <input
        ...
        className="text-xl text-gray-700 placeholder-gray-400 py-2 px-5 bg-gray-200 rounded-l-full outline-gray-300"
      />
      <button
        type="submit"
        className="text-xl text-orange-100 placeholder-gray-400 py-2 pr-5 pl-4 bg-gray-700 rounded-r-full"
      >
        <svg
      ...
      <tbody>
        {todos.length <= 0 ? (
          <tr className="odd:bg-gray-100 even:bg-gray-50">
            <td className="text-center px-1 py-2 text-gray-800" colSpan="3">
...
const TodoItem = ({ todo, index, toggleTodo, deleteTodo }) => {
  const handleToggle = async () => {
    ...
    const response = await axios.put(`http://localhost:8004/todos/${todo._id}`, updatedTodo, { withCredentials: true });
    toggleTodo(index, response.data.completed);
  };

  const handleDelete = async () => {
    await axios.delete(`http://localhost:8004/todos/${todo._id}`, { withCredentials: true });
    deleteTodo(index);
  };
  ...
  return (
    <tr
      ...
      className={
        ...
        (!todo.completed ? ' odd:bg-gray-200 even:bg-gray-300 ' : ' bg-green-100 line-through ')
      }
    >
...
const TodoStats = ({ stats }) => {
  return (
    <small id="todo_stats" className="block mb-5 mt-0 text-xs text-gray-500">
...
class Todos extends React.Component {
  ...
  fetchTodos = async () => {
    const response = await axios.get('http://localhost:8004/todos', { withCredentials: true });
      ...
      <div className="flex min-h-screen bg-gray-800">
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="container w-full max-w-2xl">
            <div className="text-3xl text-center font-bold mb-3 uppercase text-white">Todo List</div>
            <div>
